...
class robot:
    def __init__(self, world_size, noise, scale):
        ...
        self.b2x = random.random() * world_size
        self.b2y = random.random() * world_size
    ...
    def set(self, new_a2x, new_a2y, new_b2x, new_b2y, new_orientation):
        ...
        self.b2x = float(new_b2x)
        self.b2y = float(new_b2y)
        self.x = (self.a2x + self.b2x)/2
        self.y = (self.a2y + self.b2y)/2
    ...
    def move(self, turn, forward):
        ...
        res.set(new_a2x=new_a2x, new_a2y=new_a2y, new_b2x=new_b2x, new_b2y=new_b2y, new_orientation=orientation)
    ...
    def measurement_prob(self, measurement):
        ...
        r_a1a2 = sqrt(
            (self.a2x - self.world_size / 2 - self.THREE_FEET / 2) ** 2 + (self.a2y - self.world_size / 2) ** 2)  # dist to b1
        r_a1b2 = sqrt(
            (self.b2x - self.world_size / 2 - self.THREE_FEET / 2) ** 2 + (self.b2y - self.world_size / 2) ** 2)  # dist to a1

        r_b1a2 = sqrt(
            (self.a2x - self.world_size / 2 + self.THREE_FEET / 2) ** 2 + (self.a2y - self.world_size / 2) ** 2)  # dist to b1
        r_b1b2 = sqrt(
            (self.b2x - self.world_size / 2 + self.THREE_FEET / 2) ** 2 + (self.b2y - self.world_size / 2) ** 2)  # dist to b1

        r_a2b2 = sqrt((self.a2x - self.b2x) ** 2 + (self.a2y - self.b2y) ** 2) - self.THREE_FEET

        diff = (abs(r_a2b2)*10 + abs(r_a1a2 - a1a2 / self.scale) + abs(r_a1b2 - a1b2/self.scale) + abs(r_b1a2 - b1a2/self.scale) + abs(r_b1b2 - b1b2/self.scale)) / 5

        print('diff',diff, abs(r_a2b2), abs(r_a1a2 - a1a2 / self.scale), abs(r_a1b2 - a1b2/self.scale),  abs(r_b1a2 - b1a2/self.scale), abs(r_b1b2 - b1b2/self.scale))
