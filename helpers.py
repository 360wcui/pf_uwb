...
def draw_best_guess_particle(particles, canvas, color=(255, 0, 0)):
    for p in particles:
        x = int(round(p.a2x))
        y = int(round(p.a2y))
        a2 = (int(round(p.a2x)), int(round(p.a2y)))
        b2 = (int(round(p.b2x)), int(round(p.b2y)))
        # print(x, y)
        # cv2.circle(canvas, (x, y), circle_size, color, -1)
        cv2.line(canvas, a2, b2, color, 1)
